...
- **The XLSX adapter, and the second format `repair` can write.** `mirsam
  ...
  container: `sheetView/@rightToLeft` decides which side column A sits on,
  which is what a table's direction states in the other two formats — reported
  only where the sheet holds text in two or more columns, on the reasoning that
  made a text body a `Columns` container at `numCol >= 2` and not below it. A
  cell's `readingOrder` and `horizontal` are its own word, `cellStyleXfs` the
  named style behind them, and the sheet supplies the reading order to any cell
  that states none — which makes Excel the first format where a container is
  also the chain above its cells, and it is the format's rule rather than the
  adapter's.
  ...
  `dir="auto"` asks for, and the second is Excel's default chosen by the cell's
  data type rather than an edge anybody stated. A formula cell produces no unit
  — its `<v>` is a cache Excel recomputes on open — and one whose cache carries
  Arabic is named in `sources.unread`, which is
  [ADR 0009](docs/adr/0009-a-source-the-adapter-could-not-read-is-part-of-the-report.md)
  reaching a second format.

  **The writer had to append where every previous one edited.** A cell's
  formatting is not in the cell: `<c s="3"/>` names a record that forty cells
  may share, so a repair clones that record, changes the one attribute and
  repoints that cell's `@s` — every other cell keeps what it had, byte for
  byte. Shared strings work the same way, cloned so their rich-text runs
  survive. `<f>` formulas and `xl/workbook.xml`'s `<definedNames>` come through
  untouched, and mechanically so. Excel has no language slot on a cell and no
  list feature at all, so `SetLanguage` and `ConvertLiteralBullet` are refused
  and appear in `repairs.skipped` — listed, never claimed. See
  [`docs/PLAN.md`](docs/PLAN.md) §5.3.

- **Two corpus workbooks**, `quarterly-figures.xlsx` and
  `quarterly-figures-correct.xlsx`, generated by
  `scripts/make-excel-fixture.py` and regenerated with `make fixtures`. The
  first carries one instance of every defect the adapter can see plus a formula
  and two defined names; the second is the workbook the tool must leave
  completely alone. Both validate against the published ECMA-376 schemas, and
...
- **The conformance suite states a table of two cells where it stated one.** A
  ...
  grid is a container only where the sheet has more than one column of text. The
  four cases that stated a degenerate table now state a real one, which is a
  situation that proves more rather than less.
- **Four refusals join the committed list, and one assertion widens.** A native
  list and two paragraphs in different languages are Excel's alone to refuse;
  a direction-relative alignment and an unevenly filled pair of font slots are
  now refused by Excel beside PowerPoint and HTML respectively.
  `alignment-incoherent` is asserted on three formats where it was asserted on
  two — `.xlsx` can write a hard left edge, and reports it identically.
