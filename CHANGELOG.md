...
- **Font coverage.** `mirsam-core::coverage` asks the question underneath
  ...
  it as a row of empty boxes, and no shaping table would have saved it. They
  are different defects with different repairs, so they are reported
  separately. Each missing character comes back with its Unicode name,
  `U+067E ARABIC LETTER PEH`, and how many times it occurs — one letter
  repeated forty times is a heavier defect than forty appearing once.
- A font is judged **only over the characters it answers for**. The Latin, the
  European digits and the ASCII punctuation of a mixed paragraph belong to the
  other font slot, so reporting an Arabic font for having no `Q` would be
  reporting a font for text it was never asked to draw. Format characters are
  excluded because they are meaning rather than shape — no font draws U+0600
  ARABIC NUMBER SIGN — and presentation forms because pre-shaped text is
  already a blocking defect with a repair of its own, and a second finding
  there would be the tool arguing with itself about one character.
- **`mirsam-fonts`**, the adapter behind the new `ports::FontSource` port:
  which file on this machine draws the typeface a document names. It indexes
  the platform's font directories by reading each file's naming table alone
  rather than the file, which is 690 families in 118 ms on macOS instead of
  half a gigabyte of outlines. A family nobody has is an answer, not an error.
  Two things it learned from real fonts rather than from the specification:
  macOS states English family names in Mac Roman and keeps the Unicode records
  for the localisations, so refusing the legacy encoding leaves
  `Helvetica.ttc` nameless; and a family is spread over many files that all
  state the family name, so `Arial` must resolve to the *regular* face —
  taking the first match gives `Arial Bold Italic.ttf`, which on macOS has no
  Arabic while `Arial.ttf` has.
- **A fourth hand-built font**, `bold.ttf`: `Mirsam Joining` claimed a second
  time, sorting before `joining.ttf` and shaping nothing, so the regular-face
  rule fails a committed fixture rather than a user's deck.
- **`mirsam-core::charname`**, the Arabic script's Unicode names, generated by
  `scripts/make-char-names.py` from the UCD Python ships — no network, no
  third-party module — and refreshed by `make names`. It answers nothing
  outside the four logical-order Arabic blocks rather than inventing a name,
  the same refusal `joining` makes for the letters its table does not state.
- **Shaping.** `mirsam-core::joining` states, from logical-order text alone
  and with no font in the room, which contextual form each Arabic letter is
  required to take — the Unicode Joining_Type property and the rule the
  standard states over it. `mirsam-core::shape` hands each Arabic run to
  `rustybuzz` and reports what a real font actually produced. The gap between
  them is a defect no document format can express and no amount of reading XML
  will find: correct Unicode, correct direction, correct alignment, rendered
  as a row of disconnected letters.
- The shaping modules **report and do not judge**, because one letter drawn
  standalone is not a defect: macOS's Arial gives no final form to reh or meem
  and renders Arabic perfectly, since the stroke that joins to a reh belongs
  to the letter before it. What survives is the aggregate — a font with no
  shaping tables produces *no* joins in a run that required several — and
  turning that into a finding is the rule work still to come.
  [ADR 0008](docs/adr/0008-a-standalone-letter-is-not-a-shaping-defect.md).
- **Three hand-built fonts**, written byte by byte by
  `scripts/make-shaping-fixture.py` with nothing but Python's `struct` and
  regenerated by `make fonts`. They differ in one thing each: `joining.ttf`
  carries all three form features, `nonjoining.ttf` has no `GSUB` at all, and
  `partial.ttf` is Arial's behaviour reduced to its principle — so a check
  that regressed to a per-letter verdict fails against a committed fixture
