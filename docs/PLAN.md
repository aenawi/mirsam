...
relationships to parts that were not in the package. They are regenerated and
now validate, so the check can be *asked* of them; it still has to be **run**
by a person, and until it is, the M1 application check stands verified on
`quarterly-report.pptx` alone. The same session saw that
...
(`ppt/media/صورة.png`), and four compression settings across its 25 entries.
